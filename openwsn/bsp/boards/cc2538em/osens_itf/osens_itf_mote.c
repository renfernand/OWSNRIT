...
#define SONOMA_TRIMER_VA_RMS	   1  // 0.0015	// [S.23]
#define SONOMA_TRIMER_IA_RMS	   1  //0.0015	// [S.23] (NÃO CONFERIDO)
#define SONOMA_TRIMER_WATT_A	   1  // 0.0015	// [S.23] (NÃO CONFERIDO)

uint32_t CalculaInt(uint8_t rx[]);
float CalculaS23(uint8_t *rx);
float CalculaS21(uint8_t *rx);
float CalculaSType(uint8_t *paux, int stype);
...
static uint8_t osens_mote_sm_func_run_sch(osens_mote_sm_state_t *st) {
   ...
   // fval[0] = CalculaSType((uint8_t *) &param[0],23) / SONOMA_TRIMER_VA_RMS;
   // fval[1] = CalculaSType((uint8_t *) &param[1],23) / SONOMA_TRIMER_VA_RMS;
   // fval[2] = CalculaSType((uint8_t *) &param[2],23) / SONOMA_TRIMER_VA_RMS;

    sensor_points.points[0].value.value.fp32 = fval[0];
    sensor_points.points[1].value.value.fp32 = fval[1];
    sensor_points.points[2].value.value.fp32 = fval[2];
...
// Deve ser passado o endereço do primeiro dos 3 bytes com valor.
float CalculaS21(uint8_t *paux)
{
	float retorno = 0;
	uint8_t auxbuf[3];
    uint8_t *rx= &auxbuf[0];
