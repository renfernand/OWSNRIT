/*
 * Protocolo AMCA da camada MAC baseado no mecanismo RIT assincrono
...
port_INLINE void AMCAactivity_tx00(uint8_t elepending, uint8_t newtxframe);
port_INLINE uint8_t AMCAactivity_tx01(sRITelement *pmsgout,uint8_t txpending,uint8_t newtxframe);
port_INLINE void AMCAactivity_tx02(void);
port_INLINE void AMCAactivity_tx03(void);
port_INLINE void AMCAactivity_tx40(open_addr_t* address);
port_INLINE uint8_t AMCAactivity_tx45(void);
port_INLINE void AMCAactivity_tx06(void);
port_INLINE void AMCAactivity_tx07(PORT_RADIOTIMER_WIDTH capturedTime);
port_INLINE void AMCAactivity_tx08(void);
port_INLINE void AMCAactivity_tx81(void);
port_INLINE void AMCAactivity_txe08(void);
port_INLINE void AMCAactivity_tx09(PORT_RADIOTIMER_WIDTH capturedTime);
port_INLINE void AMCAactivity_txe03(void);
port_INLINE void AMCAactivity_tx04(PORT_RADIOTIMER_WIDTH capturedTime);

// RX
port_INLINE void AMCAactivity_rx00(void);
port_INLINE void AMCAactivity_rxe01(void);
port_INLINE void activityrx_waitolaecho(void);
port_INLINE void AMCAactivity_rx01(void);
port_INLINE void AMCAactivity_rx02(PORT_RADIOTIMER_WIDTH capturedTime);
port_INLINE void AMCAactivity_rx03(void);
port_INLINE void AMCAactivity_rxe03(void);
port_INLINE void AMCAactivity_rx04(PORT_RADIOTIMER_WIDTH capturedTime);
port_INLINE uint8_t AMCAactivity_rx05 (uint8_t ackRequested,uint8_t frameType, open_addr_t *rxaddr_dst);
port_INLINE void AMCAactivity_rx06(PORT_RADIOTIMER_WIDTH capturedTime);
port_INLINE void AMCAactivity_rx07(void);
port_INLINE void AMCAactivity_rxe07(void);
port_INLINE void AMCAactivity_rx08(PORT_RADIOTIMER_WIDTH capturedTime);
...
//	macRITDataWaitDuration = TICK_MAC_RIT_RX_WIND_PERIOD;
   ...
   opentimers_start(AMCA_MULTICHANNELHELLO_MS, TIMER_PERIODIC, TIME_MS, (opentimers_cbt) isr_multichannelhello);
...
void isr_ieee154e_newSlot() {
 // uint8_t ret;
  ...
  activity_ti1ORri1();
#else
  radio_setTimerPeriod(RX_AMCA_PERIOD_MS);
#endif
}
...
port_INLINE void activity_ti1ORri1() {
   ...
   if ((txpending == TRUE) || (ieee154e_vars.dataToSend != NULL)) {   // I have a packet to send
		...
		AMCAactivity_tx00(txpending,newtxframe);

		radio_setTimerPeriod(TX_AMCA_PERIOD_MS);
		...
		if (livelistretries > 0) {
			radio_setTimerPeriod(TX_AMCA_PERIOD_MS*1.5);
		}
		else{
			radio_setTimerPeriod(TX_AMCA_PERIOD_MS);
		}
		...
		rxritperiod = (RX_AMCA_PERIOD_MS + random);
...
port_INLINE uint8_t activityrx_reopenrxwindow(open_addr_t actualsrcaddr) {
   ...
   deltams = calcdeltatime(RX_AMCA_TIMEOUT_MS);
...
port_INLINE void AMCAactivity_rx00(void) {
	...
	changeState(S_AMCA_TXOLAPREPARE);
	...
	rxritperiod = (RX_AMCA_TXOLAPREPARE_MS);  // + random);
...
#if 0 //((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_RX == 1))
{
  ...
  radiotimer_schedule(RX_AMCA_TXOLAECHO_MS);
...
port_INLINE void AMCAactivity_rx02(PORT_RADIOTIMER_WIDTH capturedTime) {
	...
	changeState(S_AMCA_RXDATAPREPARE);
...
#if 0// ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_RX == 1))
{
	...
	radiotimer_schedule(RX_AMCA_TIMEOUT_MS);

#else
	radiotimer_schedule_us(RX_AMCA_DELAY_TX2RX_US);
#endif
}
 ...
 */
port_INLINE void AMCAactivity_rxe03() {
    ...
    // change state
    changeState(S_AMCA_RXDATATIMEOUT);
...
#if 0// ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_RX == 1))
{
	...
	openserial_printStatus(STATUS_RFF,(uint8_t*)&rffbuf,pos);
	   ...
	   if (ieee802514_header.frameType != IEEE154_TYPE_OLA) {
		   discardframe = AMCAactivity_rx05(ieee802514_header.ackRequested,ieee802514_header.frameType, &actualdstaddr);
	   }
		   ...
		   if ((ieee154e_vars.dataReceived->packet[10] == CMDLIVELIST) &&
				...
				if (ieee154e_vars.dataReceived->packet[11] == 0) {
					...
					changeState(S_AMCA_LLTXACKOFFSET);
					radiotimer_schedule(TX_AMCA_DELAYRXTX_MS);
...
/* AMCAactivity_rx05 - trata msg recebida diferente de ola
...
#if ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_RX == 1))
{
	...
	openserial_printStatus(STATUS_RFF,(uint8_t*)&rffbuf,pos);
		...
		if (ackRequested==1) {
    		...
    		AMCAactivity_rx06(capturedTime);
...
#endif		DiscardAndEndSlot = activityrx_reopenrxwindow(actualsrcaddr);
	}
...
port_INLINE void AMCAactivity_rx06(PORT_RADIOTIMER_WIDTH capturedTime) {
   ...
   // change state
   changeState(S_AMCA_TXACKPREPARE);
   ...
   //radiotimer_schedule(RX_AMCA_DELAY_TX_TO_RX_MS); //este valor esta bom...mas quero melhorar...
   radiotimer_schedule_us(RX_AMCA_DELAY_TX2RX_US);
 ...
 */
#if (ENABLE_LIVELIST_ACK == 1)
port_INLINE void AMCAactivity_rx61(void) {
   ...
   incroute(0x81);
	...
	//Descubro o melhor canal do src deste frame para enviar o ack
	ieee154e_vars.freq = macneigh_getBChan(&actualsrcaddr);
    radio_setFrequency(ieee154e_vars.freq);
...
#if ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_RX == 1))
{
	...
	radiotimer_schedule(RX_AMCA_TXACKEND_MS);
}
#endif
...
port_INLINE void AMCAactivity_rxe07() {
   ...
   changeState(S_AMCA_TXACKERROR);

   RECORD_RADIO_TX_OFF(ieee154e_vars.lastCapturedTime)

	//REPROGRAMAR A JANELA COM UM TEMPO MENOR...
	timeelapsedms = ieee154e_vars.lastCapturedTime + RX_AMCA_TXACKEND_MS + 10;
	if (RX_AMCA_TIMEOUT_MS > timeelapsedms){

		deltams = RX_AMCA_TXACKEND_MS + 10;
...
port_INLINE void StartTxMultiChannelHello (uint8_t pending) {
	...
	changeState(S_AMCA_RXOLAOFFSET);
...
port_INLINE void AMCAactivity_tx00(uint8_t txpending,uint8_t newtxframe) {
    ...
    if (ieee154e_vars.dataToSend->l2_frameType == IEEE154_TYPE_DATA) {
		macRIT_Pending_TX_frameType = AMCAactivity_tx01(&element,txpending,newtxframe);
	}
...
port_INLINE void AMCAactivity_tx02(void) {
    ...
    changeState(S_AMCA_RXOLAPREPARE);
    ...
    if (sRIT_vars.destaddr.type == 0){
		openserial_printError(COMPONENT_IEEE802154E,ERR_WRONG_RIT_CHANNEL,
							(errorparameter_t)S_AMCA_RXOLAPREPARE,0x02);
		endSlot();
		return;
    }
	...
	if (isBroadcast == TRUE){
		if (macRITstate == S_RIT_TX_state)  {
			...
			ret = macneigh_getBChanMultiCast(&actualbestchannel,&ieee154e_vars.targetaddr, TRUE);
			if (ret == FALSE) {
			...
			macneigh_getBChanMultiCast(&actualbestchannel,&ieee154e_vars.targetaddr, FALSE);
		}
	}
	else{
		ieee154e_vars.targetaddr = sRIT_vars.destaddr;
		actualbestchannel = macneigh_getBChan(&sRIT_vars.destaddr);
	}

	if ((actualbestchannel == 0) || (actualbestchannel > 26)){
		 // jump to the error code below this do-while loop
		openserial_printError(COMPONENT_IEEE802154E,ERR_WRONG_RIT_CHANNEL,
							(errorparameter_t)S_AMCA_RXOLAPREPARE,
							ieee154e_vars.freq);
		 endSlot();
		 return;
	}
  ...
  changeState(S_AMCA_RXOLA);
  radio_rxNow();
  radiotimer_schedule(TX_AMCA_TIMEOUT_MS);
#else
	radiotimer_schedule(TX_AMCA_RXOLAPREPARE_MS);
#endif
...
port_INLINE void AMCAactivity_tx03() {
...
port_INLINE void AMCAactivity_txe03() {
...
port_INLINE void AMCAactivity_tx04(PORT_RADIOTIMER_WIDTH capturedTime) {
	...
	changeState(S_AMCA_RXNEWOLA);

	incroute(0x04);

	rffcount = openqueue_getNrBusyPacketBuffer(COMPONENT_IEEE802154E);
...
#if ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_TX == 1))
{
	...
	if (macRITstate != S_RIT_multichnhello_state) {
		...
		if (ieee802514_header.frameType == IEEE154_TYPE_OLA) {
			...
			AMCAactivity_tx40(&actualsrcaddr);
		}
			...
			//reopenwindow = TRUE;
			incroute(0x44);
			changeState(S_AMCA_TX_CONTINUEWAIT);
			radiotimer_schedule(TX_AMCA_DELAYCONTWAIT_MS);
		}
...
#if ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_TX == 1))
{
	uint8_t   pos=0;
		...
		rffbuf[pos++]= pauxframe[2];   //802154.FCF [1]
		rffbuf[pos++]= pauxframe[3];   //asn [6]
		rffbuf[pos++]= pauxframe[4];   //asn [6]
		rffbuf[pos++]= pauxframe[5];   //asn [6]
		rffbuf[pos++]= pauxframe[6];   //asn [6]
		rffbuf[pos++]= pauxframe[7];   //asn [6]
		rffbuf[pos++]= pauxframe[8];   //asn [6]
		rffbuf[pos++]= pauxframe[9];   //asn [6]
		rffbuf[pos++]= pauxframe[10];   //asn [6]
		rffbuf[pos++]= pauxframe[11];   //asn [6]
...
port_INLINE void AMCAactivity_tx40(open_addr_t* address){
	...
	if (sRIT_vars.isBroadcastMulticast){  //FRAME RPL.DIO ou MULTICHANNELHELLO
		...
		if (macisThisAddressPendingBroadcast(address) == TRUE){
            //ritstat.txdio.countdatatx++;
			incroute(0x40);
			changeState(S_AMCA_TXDATAOFFSET);
			radiotimer_schedule(TX_AMCA_DELAYRXTX_MS);
		}
		else {
...
#if ((ENABLE_DEBUG_RFF ==1)  && (DBG_IEEE802_TX == 1))
{
	...
	if (macRITstate != S_RIT_multichnhello_state) {
		...
		pos = printaddress(*address,rffbuf,pos);
		//pos = printvar((uint8_t *)&ieee154e_vars.lastCapturedTime,sizeof(uint32_t),rffbuf,pos);
		//pos = printvar((uint8_t *)&deltams,sizeof(uint32_t),rffbuf,pos);
...
port_INLINE uint8_t AMCAactivity_tx45(void) {
	...
	deltams = calcdeltatime(TX_AMCA_TIMEOUT_MS);
...
port_INLINE void AMCAactivity_tx05(PORT_RADIOTIMER_WIDTH capturedTime) {

    radiotimer_cancel();


    changeState(S_AMCA_TXDATAREADY);
...
port_INLINE void AMCAactivity_tx06(void) {
    ...
    radiotimer_schedule(TX_AMCA_TXDATAECHO_MS);

}


port_INLINE void AMCAactivity_txe06(void) {
	...
	changeState(S_AMCA_TXDATAERROR);
	...
	if (sRIT_vars.isBroadcastMulticast == TRUE)
	{
		...
		timeelapsedms = ieee154e_vars.lastCapturedTime + TX_AMCA_TXDATAECHO_MS + 10;
		if (TX_AMCA_TIMEOUT_MS > timeelapsedms){

			deltams = TX_AMCA_TXDATAECHO_MS + 10;
...
port_INLINE void AMCAactivity_tx07(PORT_RADIOTIMER_WIDTH capturedTime) {
    ...
    changeState(S_AMCA_RXACKOFFSET);
...
#if (ENABLE_LIVELIST_ACK == 1)
	   AMCAactivity_tx08();
	   //radiotimer_schedule(RX_AMCA_DELAY_TX2RX_MS);
	   endslot = FALSE;
#else
		//update statistics
...
#if ((ENABLE_DEBUG_RFF == 1)  && (DBG_IEEE802_TX == 1))
{
	...
	uint16_t  myRank = neighbors_getMyDAGrank();
	uint8_t pos=0;
	if (macRITstate != S_RIT_multichnhello_state) {
		rffbuf[pos++]= RFF_IEEE802_TX;
		rffbuf[pos++]= 0x72;
		rffbuf[pos++]= nrPending;
		rffbuf[pos++]= ieee154e_vars.dataToSend->l2_numTxAttempts;
		pos = printvar((uint8_t *)&myRank,sizeof(uint16_t),rffbuf,pos);
		pos = printaddress(ieee154e_vars.targetaddr,&rffbuf[0],pos);
...
port_INLINE void AMCAactivity_tx08(void) {
   ...
   // change state
   changeState(S_AMCA_RXACKPREPARE);
...
#if 1
   // change state
   changeState(S_AMCA_RXACK);
   RECORD_RADIO_RX_ON
   radio_rxNow();
   radiotimer_schedule(TX_AMCA_ACK_TIMEOUT_MS);
#else
   //radiotimer_schedule_us(RX_AMCA_DELAY_TX2RX_US);
   //radiotimer_schedule(RX_AMCA_DELAY_TX_TO_RX_MS);
#endif
}
...
port_INLINE void AMCAactivity_txe08() {
...
//RX
   case S_AMCA_TXOLAPREPARE:
		  AMCAactivity_rx01();
		 break;
	  case S_AMCA_TXOLA:
		  AMCAactivity_rxe01();
		 break;
	  case S_AMCA_RXDATAPREPARE:
		 AMCAactivity_rx03();
	   	 break;
	  case S_AMCA_RXDATA:
		  AMCAactivity_rxe03();
		  break;
      case S_AMCA_TXACKPREPARE:
    	  AMCAactivity_rx07();
         break;
      case S_AMCA_TXACK:
    	 AMCAactivity_rxe07();
         break;
#if (ENABLE_LIVELIST_ACK == 1)
	  case S_AMCA_LLTXACKOFFSET:
		  AMCAactivity_rx61();
		 break;
#endif

//TX
  	  case S_AMCA_RXOLAOFFSET:
  		 AMCAactivity_tx02();
  		 break;
  	  case S_AMCA_RXOLAPREPARE:
		 AMCAactivity_tx03();
		 break;
 	  case S_AMCA_RXOLA:
 		  AMCAactivity_txe03();
 		  break;
 	  case S_AMCA_TXDATAOFFSET:
		  AMCAactivity_tx05(0);
		  break;
 	  case S_AMCA_TX_CONTINUEWAIT:
 		  AMCAactivity_tx45();
		  break;
	  case S_AMCA_TXDATAREADY:
	     AMCAactivity_tx06();
		 break;
	  case S_AMCA_TXDATA:
		 AMCAactivity_txe06();
		 break;
	  case S_AMCA_RXACKOFFSET:
		 AMCAactivity_tx08();
		 break;
	  case S_AMCA_RXACKPREPARE:
		 AMCAactivity_tx81();
		 break;
	  case S_AMCA_RXACK:
		 AMCAactivity_txe08();
...
void ieee154e_endOfFrame(PORT_RADIOTIMER_WIDTH capturedTime) {
    ...
    switch (ieee154e_vars.state) {
     case S_AMCA_TXOLA:
		 AMCAactivity_rx02(capturedTime);
		 break;
	 case S_AMCA_RXDATA:
		 AMCAactivity_rx04(capturedTime);
		break;
	 case S_AMCA_TXACK:
		 AMCAactivity_rx08(capturedTime);
		break;
//TX
	 case S_AMCA_RXOLA:
		 AMCAactivity_tx04(capturedTime);
		break;
	 case S_AMCA_TXDATA:
		 AMCAactivity_tx07(capturedTime);
		break;
	 case S_AMCA_RXACK:
		 AMCAactivity_tx09(capturedTime);
		break;
	 default:
		// log the error
