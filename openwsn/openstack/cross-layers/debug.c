...
#if IEEE802154E_RITMC
extern uint16_t ritmc_txolaack;
extern uint16_t ritmc_rxolaack;
extern uint16_t ritmc_rx04err;
extern uint16_t rx07frameOK;
extern uint16_t rx07NotLiveList;
extern uint16_t rx07FrameNotOk;
...
extern uint16_t u16Delta1;
extern uint16_t u16Delta2;
extern uint32_t synccapttime[2];
extern uint32_t slotcurrenttime;
...
/*
						rffbuf[pos++]=0xcc;
						pos = printvar((uint8_t *)&ecwvars.ecw1occurs,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.ecw2occurs,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.rxcwoccurs,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.ecwflag,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.ecwstatus,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.ecwstatus2,sizeof(uint16_t),rffbuf,pos);
						pos = printvar((uint8_t *)&ecwvars.ecw2DataTimeout,sizeof(uint16_t),rffbuf,pos);

						//pos = printvar((uint8_t *)&ecwvars.tx0aTimeout,sizeof(uint16_t),rffbuf,pos);
						//pos = printvar((uint8_t *)&ecwvars.tx0aFrameNotOk,sizeof(uint16_t),rffbuf,pos);

						rffbuf[pos++]=0xee;
						//pos = printvar((uint8_t *)&synccapttime[0],sizeof(uint32_t),rffbuf,pos);
						//pos = printvar((uint8_t *)&synccapttime[1],sizeof(uint32_t),rffbuf,pos);
						//pos = printvar((uint8_t *)&synccapttime[2],sizeof(uint32_t),rffbuf,pos);
						rffbuf[pos++]=0xee;
						...
						//pos = printvar((uint8_t *)&ritstat.counttxidletime,sizeof(uint16_t),rffbuf,pos);
 */
						for (i=0;i<MAXNUMMACNEIGHBORS;i++) {
...
#if (IEEE802154E_RITMC == 1)
//						pos = printvar((uint8_t *)&ritmc_rxolaack,sizeof(uint16_t),rffbuf,pos);
//						pos = printvar((uint8_t *)&rx07frameOK,sizeof(uint16_t),rffbuf,pos);
//						pos = printvar((uint8_t *)&ritmc_rx04err,sizeof(uint16_t),rffbuf,pos);
//						pos = printvar((uint8_t *)&rx07NotLiveList,sizeof(uint16_t),rffbuf,pos);
//						pos = printvar((uint8_t *)&rx07FrameNotOk,sizeof(uint16_t),rffbuf,pos);
			...
			pos = printvar((uint8_t *)&ritstat.txola.countdatatxok,sizeof(uint16_t),rffbuf,pos);
			pos = printvar((uint8_t *)&ritstat.txola.countacktxrxok,sizeof(uint16_t),rffbuf,pos);
#if 0 //(IEEE802154E_RITMC == 1)
			pos = printvar((uint8_t *)&txwaitola,sizeof(uint32_t),rffbuf,pos);
			pos = printvar((uint8_t *)&slotcurrenttime,sizeof(uint32_t),rffbuf,pos);
            pos = printvar((uint8_t *)&ritstat.counttxidletime,sizeof(uint16_t),rffbuf,pos);
